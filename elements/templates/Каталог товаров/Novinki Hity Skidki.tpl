name:Новинки Хиты Скидки
description:Novelty Hits Discounts
======
<!DOCTYPE html>
<html lang="ru">
	<head>
				...
				<div class="primary-block uk-width-1-1 uk-width-medium-3-4">
					...
					<div id="product_cont">
						<div class="catalog-plitka uk-grid uk-grid-width-1-1 uk-grid-width-medium-1-2 uk-grid-width-large-1-3">
							[[DocLister? &id=`product` &parents=`[*parent*]` &depth=`4` &tvPrefix=`` &tvList=`product_prev,novelty,sale,sale_persent,bestseller,articul,new_price,old_price,img_prew_prod` &tpl=`product_plitka_tpl` &orderBy=`menuindex ASC` &total=`12` &filters=`AND(content:template:in:38,64;tvd:[*novelty_sale_bestseller*]:=:1)` &prepare=`class_order_add`]]
						</div>
						[[if? &is=`[[filter_prod_count? &tv_name=`[*novelty_sale_bestseller*]` &tv_value=`1`]]:>:12` &then=`
						<div class="more-prod">
							<a href="#" start='12' class="primary-button">Показать еще</a>
						</div>
						`]]
					</div>	
					<div id='product_site'  start='12' parent='[*parent*]' ajax_url='[(site_url)][~84~]'  type='table' filter='[*novelty_sale_bestseller*]' sort='menuindex' sortdir='ASC' total='12'></div>	
