name:Проект подробнее версия 2.0 (Набор 3)
...
[[DocLister? &id=`price` &parents=`14` &depth=`0` &tvPrefix=`` &tvList=`file_price` &tpl=`price_tpl` &orderBy=`menuindex ASC` &total=`all` &noneWrapOuter=`0` &filters=``]]
					...
					<!--@- Выполненные проекты -@-->
					{{card_completed_projects? &block_name=`Выполненные проекты` &parent_id=`366`}}
					...
					<!--@- Услуги -@-->
					{{card_services? &block_name=`Другие услуги` &parent_id=`371`}}
