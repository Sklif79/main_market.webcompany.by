name:eFilterResult
description:<strong>0.1</strong> Вывод отфильтрованных товаров. Не обновлять.
======
<?php
<?php
//импортировать общие параметры из модуля eLists
//Переписан. Не обновлять	
...
/***Получение шаблона из куки***/
if(isset($_COOKIE["filter_template"])){
	$tovarChunkName = $modx->db->escape($_COOKIE["filter_template"]);
}
/******/
...
/*параметры сортировки и вывода из cookie*/
if(isset($_COOKIE["filter_display"])){
	$display = $modx->db->escape($_COOKIE["filter_display"]);
}

if(isset($_COOKIE["filter_sortby"])){
	$sortBy = $modx->db->escape($_COOKIE["filter_sortby"]);
}

if(isset($_COOKIE["filter_sortdir"])){
	$sortOrder = $modx->db->escape($_COOKIE["filter_sortdir"]);
}
/*Конец получения параметров из cookie*/
