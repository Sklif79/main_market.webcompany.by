    ...
    public function makeFilterArrays()
    {
        foreach ($this->filter_param['fieldValue'] as $k => $v) {
            if ($v['fltr_yes'] == '1') {
                ...
                $this->filters[$v['param_id']]['description'] = $v['cat_description'];
            }
            if ($v['list_yes'] == '1') {
                    ...
                    switch ($filters[$tv_id]['type']) {
                        case '1'://чекбоксы
                            ...
                            foreach ($filter_values_full[$tv_id] as $k => $v) {
                                ...
                                $description = '';
                                $description_val = '';
                                $description = (string)$filters[$tv_id]['description'];
...
EOT;
                                ...
                                if ($this->params['removeDisabled'] == '0' || $disabled == '') {
                                    ...
                                    $wrapper .= $k != '' ? $this->parseTpl(
                                        array('[+tv_id+]', '[+value+]', '[+name+]', '[+selected+]', '[+disabled+]',
                                            '[+count+]', '[+iteration+]'),
                            ...
                            $output .= $this->parseTpl(
                                array('[+tv_id+]', '[+name+]', '[+wrapper+]', '[+active_block_class+]', '[+description+]'),
                                array($tv_id, $filters[$tv_id]['name'], $wrapper, $active_block_class, $description_val),
                                $tplOuter
                            );
                            break;
...
EOT;
                        ...
                        case '9'://чекбоксы-кнопки
                            $tplRow = $tplRowCheckboxBtn;
                            $tplOuter = $tplOuterCheckboxBtn;
                            $i = 0;
                            $active_block_class = '';
                            foreach ($filter_values_full[$tv_id] as $k => $v) {
                                $tv_val_name = isset($tv_elements[$tv_id][$k]) ? $tv_elements[$tv_id][$k] : $k;
...
EOT;
    ...
    public
    function getFilterValues($content_ids, $filter_tv_ids = '')
    {
        $filter_values = array();
    ...
    public
    function getFilterFutureValues($curr_filter_values, $filter_tv_ids = '')
    {
        $filter_values = array();
    ...
    public
    function makeAllContentIDs($DLparams)
    {
        $this->content_ids = '';
    ...
    public
    function makeCurrFilterValuesContentIDs($DLparams)
    {
    ...
    public
    function setPlaceholders($array = array())
    {
        if (!empty($array)) {
    ...
    public
    function prepareGetParamsOld($fp)
    {
        $out = array();
        if (is_scalar($fp) && $fp != '') {
    ...
    public
    function getDefaultTVValues($array = array())
    {
        $out = array();
    ...
    public
    function getListFromJson($json = '', $field = 'id', $separator = ',')
    {
        $out = '';
        $_ = array();
        if (!empty($json)) {
...
//возвращает список всех дочерних товаров категории плюс товаров, прикрепленных к категории тегом tagSaver через tv с id=$tv_id
    public
    function getCategoryAllProducts($id, $tv_id)
    {
